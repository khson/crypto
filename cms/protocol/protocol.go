...
//Hash 알고리즘 선택과 signature를 이용한 서명기능
//signature 가 nil일 경우 sign을 진행한다.
//Hash 알고리즘이 nil일 경우 인증서의 hash알고리즘을 사용한다.
func (sd *SignedData) AddSignerInfoWithHashAlgorithmAndSignature(chain []*x509.Certificate, signer crypto.Signer, hashFunc *crypto.Hash, siganture []byte, attributeOption int) error {
	// figure out which certificate is associated with signer.
	pub, err := x509.MarshalPKIXPublicKey(signer.Public())
	if err != nil {
		return err
	...
	digestAlgorithm.Parameters.Tag = 5
	switch hashFunc.String() {
	case "SHA-1":
		digestAlgorithm = pkix.AlgorithmIdentifier{Algorithm: oid.DigestAlgorithmSHA1}
	case "SHA-256":
	...
	signatureAlgorithm.Parameters.Tag = 5
...
//signature 가 nil일 경우 sign을 진행한다.
//Hash 알고리즘이 nil일 경우 인증서의 hash알고리즘을 사용한다.
func (sd *SignedData) AddSignerInfoWithHashAlgorithmAndSignatureWithoutSigner(publicCert *x509.Certificate, hashFunc *crypto.Hash, siganture []byte) error {

	if err := sd.AddCertificate(publicCert); err != nil {
		return err
	}
	sid, err := NewIssuerAndSerialNumber(publicCert)
	if err != nil {
		return err
	}
	var digestAlgorithm pkix.AlgorithmIdentifier
	...
	case "SHA-512":
		digestAlgorithm = pkix.AlgorithmIdentifier{Algorithm: oid.DigestAlgorithmSHA512}
	default:
		digestAlgorithm = digestAlgorithmForPublicKey(publicCert)
	}

	signatureAlgorithm, ok := oid.X509PublicKeyAlgorithmToPKIXAlgorithmIdentifier[publicCert.PublicKeyAlgorithm]
	if !ok {
		return errors.New("unsupported certificate public key algorithm")
	}
...
//signature 가 nil일 경우 sign을 진행한다.
//Hash 알고리즘이 nil일 경우 인증서의 hash알고리즘을 사용한다.
func (sd *SignedData) CreatePKCS1Message(chain []*x509.Certificate, signer crypto.Signer, hashFunc *crypto.Hash) ([]byte, error) {
	// figure out which certificate is associated with signer.
	pub, err := x509.MarshalPKIXPublicKey(signer.Public())
	if err != nil {
		return nil, err
	...
	signatureAlgorithm, ok := oid.X509PublicKeyAlgorithmToPKIXAlgorithmIdentifier[cert.PublicKeyAlgorithm]
	if !ok {
		return nil, errors.New("unsupported certificate public key algorithm")
	}
	...
	return signer.Sign(rand.Reader, smd.Sum(nil), hash)
